...
const component = {
  ...
  list: async (sdk, orgUid, filter) => {
    ...
    // get doesn't require auth, so use static method on TencentAPI
    return processTencentAPIResponse(
      await TencentAPI.listComponents(
        {
          orgUid,
  ...
  prePublish: async (sdk, componentData) => {
    if (utils.isChinaUser()) {
      const apiClient = await getTencentAPIInstance(sdk);
      return processTencentAPIResponse(
        await apiClient.prePublishComponent({ component: componentData })
      );
    }
  ...
  postPublish: async (sdk, componentData) => {
    if (utils.isChinaUser()) {
      const apiClient = await getTencentAPIInstance(sdk);
      return processTencentAPIResponse(await apiClient.postPublishComponent(componentData));
    }
